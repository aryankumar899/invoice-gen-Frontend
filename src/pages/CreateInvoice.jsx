...
import { Box, Typography, Paper, TextField, Button, Grid, IconButton, Divider, Snackbar, Alert, Chip } from '@mui/material';
...
import CompletenessCompass from '../components/CompletenessCompass';
import { itemMemory, findTwin, weaveNotes, luckySendWindow, dueDateWarning } from '../utils/invoiceMagic';
...
export default function CreateInvoice() {
  ...
  const handleDownloadPDF = async () => {
    ...
    try {
      await downloadElementAsPdf(pdfRef.current, `${invoiceData.invoiceId || 'Invoice'}.pdf`);
    } finally {
  ...
  return (
    <Box sx={{ maxWidth: 1000, mx: 'auto' }}>
      <Box sx={{ display: 'flex', flexDirection: { xs: 'column', sm: 'row' }, justifyContent: 'space-between', alignItems: { xs: 'flex-start', sm: 'center' }, gap: 2, mb: 4 }}>
        <Box>
          <Typography variant="h4" sx={{ fontWeight: 700, color: 'text.primary', mb: 0.5, fontSize: { xs: '1.5rem', sm: '2rem', md: '2.125rem' } }}>New Invoice</Typography>
      ...
      <CompletenessCompass invoiceData={invoiceData} items={items} billFrom={billFrom} bankDetails={bankDetails} upiId={user.upiId} />
      ...
      <Box sx={{ display: 'flex', flexWrap: 'wrap', gap: 1, mb: 2 }}>
        <Button
          ...
          onClick={() => {
            ...
            if (last.billFrom) setBillFrom((prev) => ({ ...prev, ...last.billFrom }));
            if (last.bankDetails) setBankDetails((prev) => ({ ...prev, ...last.bankDetails }));
            if (last.items?.length) setItems(last.items.map((item, i) => ({ ...item, id: Date.now() + i })));
            if (last.tax !== undefined) setTax(last.tax);
            showMessage('Twin forged from your last invoice. Dates and ID are fresh.', 'success');
          }}
        >
          Twin last invoice
        </Button>
        {itemMemory(history).map((mem) => (
          <Chip
            key={mem.desc}
            size="small"
            label={`${mem.desc} · ${currency}${mem.rate}`}
            onClick={() => setItems((prev) => [...prev, { id: Date.now(), desc: mem.desc, qty: 1, rate: mem.rate }])}
          />
        ))}
      </Box>
      {findTwin(history.filter((inv) => inv._id !== editId), invoiceData.clientName, total) && (
        <Alert severity="warning" sx={{ mb: 2 }}>
          Twin radar: a similar bill for {invoiceData.clientName} already exists this week.
        </Alert>
      )}
      ...
      <Box sx={{ mb: 3 }}>
        <AiInvoiceDropzone
          ...
          onExtracted={(data) => {
            if (data.billFrom) {
              setBillFrom((prev) => ({
                name: data.billFrom.name || prev.name,
                companyName: data.billFrom.companyName || prev.companyName,
                email: data.billFrom.email || prev.email,
                phone: data.billFrom.phone || prev.phone,
                address: data.billFrom.address || prev.address,
              }));
            }
            setInvoiceData((prev) => ({
              ...prev,
              ...
              invoiceId: data.invoiceId || prev.invoiceId,
            ...
            if (data.bankDetails) {
              setBankDetails((prev) => ({
                ...
                accountName: data.bankDetails.accountName || prev.accountName,
                accountNumber: data.bankDetails.accountNumber || prev.accountNumber,
                routingNumber: data.bankDetails.routingNumber || prev.routingNumber,
              }));
            }
            if (data.items?.length) setItems(data.items);
            if (data.tax !== undefined) setTax(data.tax);
            showMessage('Invoice details filled from the uploaded file.', 'success');
          }}
        />
      </Box>

      {/* Paper 1: Bill From & Invoice Details */}
      <Paper sx={{ p: { xs: 2, md: 5 }, borderRadius: 4, bgcolor: 'background.paper', border: '1px solid', borderColor: 'divider', boxShadow: 'none', mb: 4 }}>
        <Grid container spacing={{ xs: 3, md: 4 }}>
          ...
          <Grid item xs={12} md={6}>
            ...
            <TextField fullWidth placeholder="Your Name" variant="standard" value={billFrom.name} onChange={(e) => handleBillFromChange('name', e.target.value)} sx={{ mt: 1, '& .MuiInputBase-input': { fontSize: '1.1rem', fontWeight: 600 } }} InputProps={{ disableUnderline: false }} />
          ...
          <Grid item xs={12} md={6}>
            ...
            <TextField fullWidth placeholder="Client Name" variant="standard" value={invoiceData.clientName} onChange={(e) => handleChange('clientName', e.target.value)} sx={{ mt: 1, '& .MuiInputBase-input': { fontSize: '1.1rem', fontWeight: 600 } }} InputProps={{ disableUnderline: false }} />
          ...
          <Grid item xs={12} md={6} sx={{ textAlign: { md: 'right' } }}>
             ...
             <Box sx={{ display: 'flex', justifyContent: { md: 'flex-end' }, alignItems: 'center', mt: 1, gap: 2 }}>
               ...
               <TextField variant="outlined" size="small" value={invoiceData.invoiceId} onChange={(e) => handleChange('invoiceId', e.target.value)} sx={{ flex: 1, maxWidth: 200 }} />
             </Box>
      ...
      {/* Paper 2: Line Items */}
      <Paper sx={{ p: { xs: 2, md: 5 }, borderRadius: 4, bgcolor: 'background.paper', border: '1px solid', borderColor: 'divider', boxShadow: 'none', mb: 4 }}>
        <Typography variant="h6" sx={{ color: 'text.primary', mb: 4, display: 'flex', alignItems: 'center', gap: 1, fontSize: { xs: '1rem', md: '1.25rem' } }}>
        ...
        {/* Items List */}
        {items.map((item) => (
          <Box key={item.id} sx={{ mb: 2, p: { xs: 1.5, md: 2 }, bgcolor: 'action.hover', borderRadius: 2, position: 'relative' }}>
            <Grid container spacing={2} alignItems="center">
              <Grid item xs={12} md={6}>
                <TextField fullWidth placeholder="Description of service or product..." variant="standard" value={item.desc} onChange={(e) => handleItemChange(item.id, 'desc', e.target.value)} InputProps={{ disableUnderline: true, sx: { color: 'text.primary', fontWeight: 500 } }} />
              </Grid>
              <Grid item xs={4} md={2}>
                <TextField fullWidth type="number" placeholder="Qty" variant="outlined" size="small" value={item.qty} onChange={(e) => handleItemChange(item.id, 'qty', e.target.value)} />
              </Grid>
              <Grid item xs={4} md={2}>
                <TextField fullWidth type="number" placeholder="Rate" variant="outlined" size="small" value={item.rate} onChange={(e) => handleItemChange(item.id, 'rate', e.target.value)} />
              </Grid>
      ...
      <Paper sx={{ p: { xs: 2, md: 5 }, borderRadius: 4, bgcolor: 'background.paper', border: '1px solid', borderColor: 'divider', boxShadow: 'none', mb: 4 }}>
        ...
        <Grid container spacing={4} justifyContent="space-between">
          <Grid item xs={12} md={6}>
            ...
            <Box sx={{ bgcolor: 'action.hover', p: { xs: 2, md: 3 }, borderRadius: 3, mb: 4 }}>
              <Grid container spacing={2}>
                <Grid item xs={12} sm={6}>
                  <TextField fullWidth label="Bank Name" variant="standard" size="small" value={bankDetails.bankName} onChange={(e) => handleBankChange('bankName', e.target.value)} />
                </Grid>
                <Grid item xs={12} sm={6}>
                  <TextField fullWidth label="Account Name" variant="standard" size="small" value={bankDetails.accountName} onChange={(e) => handleBankChange('accountName', e.target.value)} />
                </Grid>
                <Grid item xs={12} sm={6}>
                  <TextField fullWidth label="Account Number" variant="standard" size="small" value={bankDetails.accountNumber} onChange={(e) => handleBankChange('accountNumber', e.target.value)} />
                </Grid>
                <Grid item xs={12} sm={6}>
                  <TextField fullWidth label="Routing/IFSC Code" variant="standard" size="small" value={bankDetails.routingNumber} onChange={(e) => handleBankChange('routingNumber', e.target.value)} />
                </Grid>
              </Grid>
            </Box>
            ...
            <Box sx={{ display: 'flex', flexWrap: 'wrap', gap: 1, mb: 1 }}>
              {[
                ...
                <Chip
                  ...
                  onClick={() => handleChange('notes', weaveNotes(tone.id, { ...invoiceData, currency, total }))}
                />
              ))}
            </Box>
            ...
            {dueDateWarning(invoiceData.dueDate) && (
              <Alert severity="info" sx={{ mt: 1.5 }}>{dueDateWarning(invoiceData.dueDate)}</Alert>
            )}
            <Typography sx={{ mt: 1.5, fontSize: 12, color: 'text.secondary' }}>
              Lucky send window from your history: {luckySendWindow(history).label}
            </Typography>
          </Grid>
          
          <Grid item xs={12} md={5}>
            <Box sx={{ bgcolor: 'action.hover', borderRadius: 3, p: { xs: 2, md: 3 } }}>
              <Box sx={{ display: 'flex', justifyContent: 'space-between', mb: 2 }}>
                <Typography sx={{ color: 'text.secondary', fontWeight: 600 }}>Subtotal</Typography>
                <Typography sx={{ color: 'text.primary', fontWeight: 600 }}>{currency}{subtotal.toFixed(2)}</Typography>
              </Box>
              <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 2 }}>
                <Typography sx={{ color: 'text.secondary', fontWeight: 600 }}>Tax (%)</Typography>
                <TextField type="number" variant="outlined" size="small" value={tax} onChange={(e) => setTax(e.target.value)} sx={{ width: 80, '& .MuiInputBase-input': { textAlign: 'right' } }} />
              </Box>
              <Box sx={{ display: 'flex', justifyContent: 'space-between', mb: 3 }}>
                <Typography sx={{ color: 'text.secondary', fontWeight: 600 }}>Tax Amount</Typography>
                <Typography sx={{ color: 'text.primary', fontWeight: 600 }}>{currency}{taxAmount.toFixed(2)}</Typography>
              </Box>
              <Divider sx={{ borderColor: 'rgba(255,255,255,0.1)', mb: 2 }} />
              <Box sx={{ display: 'flex', justifyContent: 'space-between' }}>
                <Typography variant="h6" sx={{ color: 'text.primary', fontWeight: 800 }}>Total</Typography>
      ...
      <Snackbar open={snackbar.open} autoHideDuration={6000} onClose={handleCloseSnackbar} anchorOrigin={{ vertical: 'bottom', horizontal: 'center' }}>
        <Alert onClose={handleCloseSnackbar} severity={snackbar.severity} sx={{ width: '100%', background: '#1f2937', color: 'text.primary' }}>
          {snackbar.message}
        </Alert>
      </Snackbar>
      ...
      <Box sx={{ position: 'absolute', top: -9999, left: -9999, zIndex: -100 }}>
        <Box ref={pdfRef} sx={{ 
          ...
          fontFamily: 'Arial, Helvetica, sans-serif', letterSpacing: '0px',
          '& *': { fontFamily: 'Arial, Helvetica, sans-serif !important', letterSpacing: '0px !important', wordBreak: 'normal' }
        }}>
          {/* Business Header */}
          <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', borderBottom: '2px solid #f3f4f6', pb: 4, mb: 5 }}>
            <Box>
              <Typography sx={{ fontSize: '2.5rem', fontWeight: 800, color: '#111827', lineHeight: 1.15 }}>INVOICE</Typography>
